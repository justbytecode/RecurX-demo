    ...
    <AlertDialog open={showPopup} onOpenChange={setShowPopup}>
      <AlertDialogContent className="max-w-md mx-auto border-0 bg-transparent shadow-none p-0 overflow-hidden">
        <div className="relative">
          ...
          <div className={`absolute inset-0 bg-gradient-to-br ${getGradient()} opacity-20 blur-xl rounded-3xl`} />
          
          {/* Main content container */}
          <div className={`relative bg-slate-900/95 backdrop-blur-xl border border-slate-600/30 rounded-3xl p-8 shadow-2xl transform transition-all duration-700 ${
            ...
            <Sparkles className="absolute top-4 right-4 w-6 h-6 text-slate-400 animate-spin" style={{ animationDuration: '3s' }} />
            ...
            <AlertDialogHeader className="text-center space-y-6 relative z-10">
              ...
              <div className="flex justify-center">
                <div className={`relative p-4 rounded-full bg-gradient-to-br ${getGradient()} shadow-xl`}>
              ...
              <AlertDialogTitle className={`text-3xl font-black text-white text-center transform transition-all duration-500 ${
              ...
              <AlertDialogDescription className="text-white text-lg font-medium">
                {currentStep === 1 ? (
                  <span className="flex items-center justify-center gap-2">
                    ...
                    Congratulations!
                    <Star className="w-5 h-5 text-amber-400 animate-bounce" style={{ animationDelay: '0.2s' }} />
                  </span>
                ) : (
                  <span className="flex items-center justify-center gap-2">
                    <Gift className="w-5 h-5 text-emerald-400 animate-bounce" />
                    Welcome to the club!
                    <Gift className="w-5 h-5 text-emerald-400 animate-bounce" style={{ animationDelay: '0.2s' }} />
                  </span>
                )}
              </AlertDialogDescription>
              ...
              <div className="flex justify-center gap-2 pt-4">
                <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  currentStep === 1 ? 'bg-slate-400 scale-125' : 'bg-slate-600'
                }`} />
                <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  currentStep === 2 ? 'bg-slate-400 scale-125' : 'bg-slate-600'
                }`} />
              </div>
            </AlertDialogHeader>

            {/* Animated border glow */}
            <div className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${getGradient()} opacity-30 blur-sm animate-pulse`} style={{ zIndex: -1 }} />
          </div>
        </div>
      </AlertDialogContent>
    </AlertDialog>
